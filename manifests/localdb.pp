# This class creates a localdb for CCS
class ccs::localdb(
  Hash $localdb_configuration,
  String $localdb_username,
  String $localdb_password,
  String $localdb_dbname,
){
  ...
  exec{ 'Verify if DB exists':
    ...
    command     => "mysql -e \"create database atsccsdb; \
                    CREATE USER '${localdb_username}'@'localhost' IDENTIFIED BY '${localdb_password}'; \
                    GRANT ALL PRIVILEGES on ${localdb_dbname}.* to '${localdb_username}'@'localhost';\""
  }
